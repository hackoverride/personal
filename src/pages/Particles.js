...
export function Particles() {
  ...
  const [camera] = useState(() => {
    ...
    const right = 1 * scale;
    const top = 1 * scale;
    const bottom = -1 * scale;
    const near = 2 / Math.pow(2, 53);
    const far = 3;
    let cam = new THREE.OrthographicCamera(left, right, top, bottom, near, far);
    cam.updateProjectionMatrix(); // Always needed after changing camera properties
    return cam;
  });
